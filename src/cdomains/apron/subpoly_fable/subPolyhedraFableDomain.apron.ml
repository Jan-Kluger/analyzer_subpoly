(** OCaml implementation of the subpolyhedra domain (Claude Fable implementation, activated as [subpoly_fable]).
    ...
    delta_L of the paper) when [ana.subpoly_fable.basis-exploration] is enabled.
...
module M = Messages
open GobApron
open SubPolyhedraFableCore

module Mpqf = SharedFunctions.Mpqf
module RationalInterval = RationalintervalFable.RationalInterval
...
struct
  ...
  let basis_exploration_enabled =
    lazy (try GobConfig.get_bool "ana.subpoly_fable.basis-exploration" with _ -> false)
...
struct
  ...
  let name () = "subpoly_fable"

  let to_yojson _ = failwith "SubPolyhedraFableDomain.to_yojson: not implemented"
  ...
  (** Structural invariant violations of a state; an empty list means the
      ...
      operation when [dbg.subpoly_fable.check-invariants] is enabled. *)
  ...
  let invariant_checking_enabled =
    lazy (try GobConfig.get_bool "dbg.subpoly_fable.check-invariants" with _ -> false)
