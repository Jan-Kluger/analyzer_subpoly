...
module RationalInterval : Intervalsig.IntervalSig with type bound = Rat.t = struct
  type bound = Rat.t
  (* TODO: make optional maybe? *)
  type t = (Rat.t option * Rat.t option)

  let equal ((l1, u1) as interval_1 : t) ((l2, u2) as interval_2 : t) =
    match interval_1, interval_2 with
    | (None, None), (None, None) -> true
    | (None, Some u1), (None, Some u2) -> Rat.equal u1 u2
    | (Some l1, None), (Some l2, None) -> Rat.equal l1 l2
    | (Some l1, Some u1), (Some l2, Some u2) -> Rat.equal l1 l2 && Rat.equal u1 u2
    | _ -> false
  ...
  let compare_bound_opt (none_is_neg_inf : bool) (a : Rat.t option) (b : Rat.t option) =
    match a, b with
    | None, None -> 0
    | None, Some _ -> if none_is_neg_inf then -1 else 1
    | Some _, None -> if none_is_neg_inf then 1 else -1
    | Some a, Some b -> Rat.compare a b
  ...
  let hash ((l, u) : t) =
    Hashtbl.hash (Option.map Rat.hash l, Option.map Rat.hash u)
  ...
  let scale_bound (factor : bound) (b : bound option) =
    Option.map (fun x -> Rat.mul factor x) b

  let scale (factor : bound) ((l, u) : t) =
    if Rat.compare factor Rat.zero < 0 then
  ...
  let add_const (c : bound) ((l, u) : t) =
    let add_opt = Option.map (fun x -> Rat.add x c) in
    add_opt l, add_opt u
  ...
  let meet ((l1, u1) : t) ((l2, u2) : t) =
    ...
    | Some l, Some u when Rat.compare l u > 0 -> None
    | _ -> Some (lower, upper)
  ...
  let lower_leq (a : bound option) (b : bound option) =
    ...
    | None, _ -> true
    | Some _, None -> false
    | Some a, Some b -> Rat.compare a b <= 0
  ...
  let show_bound (b : bound option) =
    ...
    | None -> "inf"
    | Some x ->
      if Z.equal (Rat.get_den x) Z.one then Z.to_string (Rat.get_num x)
      else Z.to_string (Rat.get_num x) ^ "/" ^ Z.to_string (Rat.get_den x)
