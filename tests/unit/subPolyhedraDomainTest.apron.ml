(* QCheck lattice-law tests for the manually implemented subpolyhedra domain
   ([SubPolyhedraDomain], activated as [subpoly]).
...
module D0 = SubPolyhedraDomain.D
module Mpqf = SharedFunctions.Mpqf

(* With SUBPOLY_TRACE set, every domain operation is logged with its
   operands before it runs, so a non-terminating operation can be identified
   from the log tail. *)
module D = struct
  include D0
  let tracing = Sys.getenv_opt "SUBPOLY_TRACE" <> None
  let trace1 name t =
