...
*)
  ...
  (* Anti-cycling guard for [maximize_rec]. The maximization pivots with a best-ratio
     rule and takes degenerate (zero-ratio) pivots greedily, with no Bland's rule and no
     iteration cap; on a degenerate LP it can therefore cycle between bases forever
     (observed: goblint's subpolyhedra reduction on a slack-heavy relational program).
     The cap bounds the pivot count well above what any non-cycling maximization of a
     problem this size needs -- it scales with the number of variables, and legitimate
     runs finish in far fewer pivots -- so it only ever fires on a cycle. On the cap the
     objective is reported as unbounded: for the sole caller (an interval refinement
     reading the max as an upper bound) that is the sound direction, yielding no finite
     bound so the stored interval is kept rather than tightened past the true optimum. *)
  let maximize_pivot_cap env = 1000 + 50 * (MX.cardinal env.basic + MX.cardinal env.non_basic)

  let rec maximize_rec env opt rnd =
    if rnd > maximize_pivot_cap env then rnd, env, Some (opt, false) (* cycling: sound give-up *)
    else begin
